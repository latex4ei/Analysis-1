...
%
	\fancyfoot[C]{von Lukas Kompatscher \& Steven Salloum\ -- Mail: \email{lukas.kompatscher@tum.de} / \email{steven.salloum@tum.de}}
...
\end{array}\end{math}\\
\\
Wichtige Zahlen: $\sqrt{2} = 1,41421$ \quad $e = 2,71828$ \quad $\pi =  3,14159$ \quad $\sqrt{2}/2 = 1/\sqrt{2} = 0.707$ \quad $\sqrt{3}/2 = 0.866$
...
\\
Jede rationale Zahl $\frac m n \in \mathbb Q$ hat eine Dezimaldarstellung.\\
...
IV: Annahme $f(n)=g(n)$ gilt für ein festes, aber beliebiges $n\in\mathbb N$\\
IS: $n \rightarrow n+1$: \quad Zeige $f(n+1)=\underset{=wahr}{f(n)}  \dotsc=g(n+1)$

\subsection{Komplexe Zahlen}
% ----------------------------------------------------------------------
Eine komplexe Zahl $z=a+b\mathbf{i},\ z\in \mathbb C, \quad a,b \in \mathbb R$ besteht aus einem Realteil $\Re(z)=a$ und einem Imaginärteil $\Im(z)=b$, wobei $\mathbf{i}=\sqrt{-1}$ die imaginäre Einheit ist.
...
$\overline{z}=a-b\mathbf{i}$\qquad \qquad ${\overline{e^{ix}}} = e^{-ix}$ für $x \in \mathbb R$  \\
...
\item[n-te Wurzel:] $w_k = \sqrt[n]{z}= \sqrt[n]{r} \left(\cos \left(\frac{\varphi + 2k\pi}{n}\right) + \mathbf{i} \sin \left(\frac{\varphi + 2k\pi}{n}\right)\right) \\ k =0,1, \ldots, n-1$
...
\textbf{Kandidaten für Extrema (lokal, global)}
...
\textbf{Mittelwertsatz:} Falls $f$ auf (a,b) diffbar, dann $\exists x_0 \in (a,b):f'(x_0)=\frac{f(b)-f(a)}{b-a}$\\
...
\end{eqnarray*}
\subsubsection{Additionstheoreme} % (fold)
...
\tan & 0 & \frac{\sqrt{3}}{3}&1 &\sqrt{3} & undef & -\sqrt{3}& -1& -\frac{\sqrt{3}}{3} & 0 & undef & 0\\
\end{array}$
}
...
\subsection{Folgen}
% ----------------------------------------------------------------------
Eine Folge ist eine Abbildung $a: \mathbb N_0 \rightarrow \mathbb R,\ n \mapsto a(n) =: a_n$\\
explizite Folge: $(a_n)$ mit $a_n=a(n)$\\ rekursive Folge: $(a_n)$ mit $a_{n+1} = f(a_n);\  Startwert\ a_0\ gegeben$\\
...
\paragraph{Es gilt:}
\begin{itemize}\itemsep0pt
\item Ist die Folge $(a_n)$ konvergent, so ist ihr Grenzwert eindeutig.
...
\item \emph{Das Cauchy-Kriterium:} Eine Folge $(a_n)$ konvergiert genau dann, wenn: \\ $\forall \epsilon >0 \, \exists \,  N \in \mathbb N_0: \abs{a_n - a_m} < \epsilon \, \forall n, m \ge N$
\end{itemize}
...
\item Gebrochen-rationale Funktion $\frac{P(x)}{Q(x)}$: im Zähler und Nenner jeweils die höchste Potenz von $x$ ausklammern.
\item Rekursive Folgen: $a_{n+1} = f(a_n)$: Zuerst Fixpunkte berechnen ($a = f(a)$). Wenn die Folge konvergiert, dann gegen einen Fixpunkt. Monotonie und Beschränktheit (z.B. per Induktion) garantieren Konvergenz, sind aber nicht notwendig. Alternierende Konvergenz ist möglich. Anfangswerte können helfen, das Verhalten abzuschätzen.
\end{itemize}
...
% ----------------------------------------------------------------------
\begin{eqnarray*}
	\underset{\text{Harmonische Reihe}}{\sum_{n=1}^\infty \frac{1}{n} = \infty} \qquad \qquad  \qquad \qquad \underset{\text{Geometrische Reihe}}{\sum_{n=0}^\infty q^n = \frac{1}{1-q}} \qquad \text{falls}\ |q|<1
\end{eqnarray*}
\begin{eqnarray*}
...
Die Reihe $\sum^{\infty}_{n = 0} a_n$ divergiert, falls $a_n \not\rightarrow 0$.

\textbf{Minorantenkriterium:} Es sei \( a_n \ge b_n \ge 0 \) für alle \( n \ge n_0 \) und die Reihe \( \sum_{n=n_0}^\infty b_n \) divergiert. Dann divergiert auch \( \sum_{n=n_0}^\infty a_n \). Die Reihe über \( b_n \) heißt divergente Minorante.

\textbf{Alternierende Reihen:} Die Reihe \( \sum^{\infty}_{n = 0}(-1)^n a_n \) konvergiert, wenn \( (a_n) \) eine monoton fallende Nullfolge ist (Leibniz-Kriterium).

\textbf{Majorantenkriterium:} Es sei \( 0 \le a_n \le b_n \) für alle \( n \ge n_0 \) und \( \sum_{n=n_0}^\infty b_n \) konvergiert. Dann konvergiert auch \( \sum_{n=n_0}^\infty a_n \). Die Reihe über \( b_n \) heißt konvergente Majorante.

\textbf{Absolute Konvergenz:} ($\sum^\infty_{n=0} |a_n|$ konvergiert), falls:\\
\textbf{1. Majorantenkriterium:}  
Falls eine konvergente Reihe \( \sum^\infty_{n=n_0} b_n \) mit \( b_n \ge 0 \) existiert, sodass $|a_n| \le b_n \quad \text{für alle } n \ge n_0$, dann ist \( \sum^\infty_{n=n_0} a_n \) absolut konvergent.
\\
\textbf{2. Quotienten- und Wurzelkriterium:} (BETRAG nicht vergessen!)
\[
\rho := \lim_{n \rightarrow \infty} \left| \frac{a_{n+1}}{a_n} \right| \quad \text{oder} \quad \rho := \lim_{n \rightarrow \infty} \sqrt[n]{|a_n|}
\]
Falls dieser Grenzwert \( \rho \) existiert, gilt:
\begin{eqnarray*}
	\text{Falls}
	\begin{cases}
...
\end{eqnarray*}
\\
Jede absolut konvergente Reihe ($\sum^\infty_{n=0} |a_n|$) ist konvergent ($\sum^\infty_{n=0} a_n$)
...
$f(x)\begin{cases}
	\text{konvergiert absolut, falls} & \abs{x-c} < R \\
	\text{divergiert, falls} & \abs{x-c} > R \\
	\text{keine Aussage möglich, falls} & \abs{x-c} = R
	\end{cases}$\\ \\
Bei reellen Reihen gilt: \\
$\Ra f(x)$ konvergiert im offenen Intervall $I=(c-R,c+R)$ \\
...
$f$ diffbar in $x_0$, falls $f$ in $x_0$ stetig und $\underset{h\rightarrow 0}{\lim}\frac{f(x_0+h)-f(x_0)}{h}=f'(x_0)$ existiert.
\\
Man beachte, dass $h\rightarrow0$ den links- und rechtsseitigen Grenzwert zusammenfasst, d.h. beide müssen existieren und übereinstimmen.
...
Potenzreihe: $f: ] \underbrace{c-R, c+R}_{\subseteq D}	 [ \rightarrow \mathbb R, f(x) = \sum_{n=0}^{\infty} a_n (x -c)^n$ \quad $\Rightarrow$ \quad $f'(x) = \sum_{n=0}^{\infty} n a_{n} (x-c)^{n-1}$\\
...
\item Partielle Integration: $\int uv'\ dx=uv-\int u'v\ dx$
...
x \operatorname{arsinh} (x) - \sqrt{x^2+1} & \operatorname{arsinh} (x) & \frac{1}{\sqrt{x^2+1}}\\
x \operatorname{arcosh} (x) - \sqrt{x^2-1}  & \operatorname{arcosh} (x) & \dfrac{1}{\sqrt{x^2-1}}\\
\frac{1}{2}\ln(1-x^2) + x \operatorname{artanh} (x) & \operatorname{artanh} (x) & \frac{1}{1-x^2}\\
...
\subsubsection{Laplace-Transformation von $f: [0,\infty[ \ra \mathbb R,\ t \mapsto f(t)$}
$\mathcal L \; f(t) = F(s) = \int\limits_{0}^{\infty} e^{-st} f(t)\ \mathrm dt = \lim\limits_{b \ra \infty} \int\limits_{0}^{b} e^{-st} f(t)\ \mathrm dt$

\subsubsection{Integration rationaler Funktionen}

Gegeben: $\int \frac{A(x)}{Q(x)} \,\mathrm dx \qquad A(x), Q(x) \in \mathbb R[x]$
\begin{enumerate}\itemsep0pt
\item Falls $\deg A(x) \ge \deg Q(x) \Ra$ Polynomdivision: \\
$\frac{A(x)}{Q(x)} = P(x) + \frac{B(x)}{Q(x)}$ mit $\deg B(x) < \deg Q(x)$

\item Faktorisiere $Q(x)$ in unzerlegbare Polynome.

\item \textbf{Partialbruchzerlegung:} \\
$\frac{B(x)}{Q(x)} = \frac{\ldots}{(x - a_n)} + \ldots + \frac{\ldots}{\ldots}$

\item Berechnung von $A, B, C, \ldots$:
\begin{itemize}
\item mit Hauptnenner multiplizieren
\item passende $x$-Werte einsetzen (Nullstellen)
\item ggf. Klammern ausmultiplizieren und Koeffizientenvergleich
\end{itemize}

\item Integriere die Summanden mit bekannten Stammfunktionen.
\end{enumerate}
...
\begin{itemize}\itemsep-2pt
	\item $\mathcal C^0$-Kurve: Positionsstetigkeit (lückenlose Kurve)
...
%===========================================================================================================================================================
Ein Skalarfeld ordnet jedem Vektor eines Vektorraums einen skalaren Wert zu.\\
